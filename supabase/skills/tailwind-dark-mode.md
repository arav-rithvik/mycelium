...
- **The flash (FOUC) is only fixable with a render-blocking inline script in `<head>`.** Any React-based approach (`useEffect`, context, a client component) runs *after* the server HTML paints, so users see the wrong theme for a frame on every load. The script must be inline (not `src=...`) and not deferred so it executes before paint.
